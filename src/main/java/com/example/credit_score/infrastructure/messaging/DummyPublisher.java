package com.example.credit_score.infrastructure.messaging;

import com.example.credit_score.domain.model.CreditEvaluation;
import com.example.credit_score.domain.port.out.CreditScorePublisher;
import org.springframework.stereotype.Component;

@Component
public class DummyPublisher implements CreditScorePublisher {
    @Override
    public void publish(CreditEvaluation evaluation) {
        System.out.println("Mock publish: " + evaluation.getCreditScore());
    }
}
